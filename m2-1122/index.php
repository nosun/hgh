...
if(!empty($_GET['load'])){
    ...
    while ($r = $empire->fetch($c_sql1)) {
        ...
        <section class="li<?= $vi1 ?>" style="height: 62px">
            <div class="img-text">
                <a  style="font-size: 15px" href="<?= $titleurl ?>"><b><?= $r[title] ?></b></a>
                <p style="font-size:13px;"><?=$passedtimestr?>    <?=$class_r[$r['classid']]['classname']?>   <?=$r['author']?></p>
            </div>
...
<?php $i = 1;while($r=$empire->fetch($c_sql))
{
    ...
    <li>
        <a class="pic" href="show.php?classid=<?=$r['classid']?>&id=<?=$r['id']?>&style=<?=$wapstyle?>&cpage=<?=$page?>&cid=<?=$classid?>&bclassid=<?=$bclassid?>"><img style="position: inherit;clip:rect(0px 110px 100px 0px)" src="<?=$r['titlepic']?>" /></a>
