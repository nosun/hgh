...
//登录附加cookie
function AddLoginCookie($r, $logincookie = NULL) {
    $userid = intval($r['userid']);
    if ($userid) {
        if (class_exists("EIService") == FALSE)//取绑定的第三方交互平台账号 LGM[2014年2月11日16:46增加]
        ...
        if (!empty($eis)) {
            foreach ($eis as $d) {                
                if (!$d['used'] && $d['id']) {//已经过期:
                    $oauth = EIService::GetOAuth($d['aid'], array('id' => $d['id']));
                    if ($oauth->SupportRefreshToken()) {
                       if ($oauth->DoRefreshToken()) {//手动续期访问令牌
                            $d['used'] = 1;                           
                        }
                    }
                }
            ...
            $rp = NULL;
            EIService::SetCUBEISForSession($rp);
        }
    }
}
...
/**
 * 返回设置短消息
 * @param int $havemsg 1为消息,2为通知,3为全部信息
 * @param int $ecms,0前台(默认),1后台
 * @return int
 */
...
function GetUserEmail($userid,$username){
	global $empire,$dbtbpre;
	$r=$empire->fetch1("select ".eReturnSelectMemberF('email')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1",MYSQL_ASSOC);
	return $r['email'];
}
/**
 * 取指定会员ID的会员名称(LGM添加[2014年3月18日11:18])
...
function GetUserName($userid){
	global $empire;
        ...
        $r=$empire->fetch1("select ".eReturnSelectMemberF('username')." from ".eReturnMemberTable()." where ".egetmf('userid')."={$userid} limit 1",MYSQL_ASSOC);
	if(empty($r)) return NULL;
        return $r['username'];
}
/**
 * 取指定会员名称的会员ID(LGM添加[2014年3月18日11:19])
 * @global resource $empire
...
function GetUserID($username){
	...
	$r=$empire->fetch1("select ".eReturnSelectMemberF('userid')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1",MYSQL_ASSOC);
        if(empty($r)) return 0;
	return intval($r['userid']);
}
//返回修改资料
function ReturnUserInfo($userid){
	global $empire,$dbtbpre;
	$r=$empire->fetch1("select ".eReturnSelectMemberF('username,email,groupid,userfen,money,userdate,zgroupid,checked,registertime').",md5(reverse(password)) as kpassword from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' limit 1",MYSQL_ASSOC);
	return $r;
}
...
/**
 * 会员退出时清空COOKIE(LGM修改[2014年3月14日22:32])
 * @param int $logincookie,0为后台(默认),1为前台,-1为不加COOKIE-KEY前缀
 */
function EmptyEcmsCookie($ecms=0){
        $set1 = 0;
	$set1 += (int)delcookie('mlusername',$ecms);
	$set1 += (int)delcookie('mluserid',$ecms);
	$set1 += (int)delcookie('mlgroupid',$ecms);
	$set1 += (int)delcookie('mlrnd',$ecms);
	$set1 += (int)delcookie('mlauth',$ecms);
        $set1 += (int)delcookie('returnurl',$ecms);//LGM增加,把转跳地址清除
        return $set1;
}
...
function qCheckLoginAuthstr(){
	...
	$re['userid']=(int)getcvar('mluserid',0,TRUE);//LGM修改,把用户ID加密[2014年3月16日21:43]
...
function islogin($uid=0,$uname='',$urnd=''){
	...
	$rnd=RepPostVar($rnd);
	$cr=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,email,groupid,userfen,money,userdate,zgroupid,havemsg,checked,registertime')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd' limit 1");//LGM修改,会员名不加判断,允许改[2014年3月19日20:29]
	if(!$cr['userid'])
	{
		EmptyEcmsCookie();
		if(!getcvar('returnurl'))
...
function DoEcmsMemberLogin($r,$lifetime=0){
	...
	esetcookie("mluserid",$r['userid'],$logincookie,0,TRUE);//LGM修改,把用户ID加密[2014年3月16日21:43]
