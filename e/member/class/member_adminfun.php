...
function admin_EditMember($add,$logininid,$loginin){
	...
	if(defined('UC_ENABLE') && UC_ENABLE) $truepassword=$add[password];//Ucenter
	//修改密码
	$add1='';
	if($add[password])
	{
...
function admin_DelMember($userid,$loginuserid,$loginusername){
	...
	//--------------------------- Ucenter ---------------------------
        if(defined('UC_ENABLE') && UC_ENABLE){
	$success=uc_user_delete($r['username']);
	if($success==0)
	{
	...
	//删除短消息
	$del=$empire->query("delete from {$dbtbpre}enewsqmsg where to_username='".mysql_real_escape_string($dousername)."'");//LGM修改,有潜在危险[2014年3月19日22:18]
	//删除购买记录
	...
	$del=$empire->query("delete from {$dbtbpre}enewsmemberfeedback where userid='$userid'");
        //删除第三方交互关联表记录,LGM增加[2014年3月14日10:46]
...
function admin_DelMember_all($userid,$logininid,$loginin){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		//--------------------------- Ucenter ---------------------------
		if(defined('UC_ENABLE') && UC_ENABLE) $delusername[]=$ur['username'];
...
function admin_ClearMember($add,$logininid,$loginin){
	...
	if($inid)
	{
		...
		$del=$empire->query("delete from {$dbtbpre}enewsmemberfeedback where ".$addaw);
                //删除第三方交互关联表记录,LGM增加[2014年3月14日10:46]
                $del=$empire->query("delete from {$dbtbpre}enewsmember_connect where userid='$addaw'");
	}
	insert_dolog("");//操作日志
