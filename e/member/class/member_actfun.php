...
/**
 * 发送取回密码邮件(LGM修改,支持静默方式,指定标题与内容[2014年3月19日8:35])
 * @global resouce $empire
 * @global string $dbtbpre
 * @global array $public_r
 * @param array $add 参数,必须包含用户名与邮箱地址
 * @param string $getpasstitle 邮件标题,空为到数据库取
 * @param string $getpasstext 邮件内容,空为到数据库取
 * @param bool $silence 是否为静默方式,如是则不弹出消息框,返回array或者true
 * @return boolean
 */
function SendGetPasswordEmail(array &$add,$getpasstitle=NULL, $getpasstext=NULL,$silence=FALSE){
	global $empire,$dbtbpre,$public_r;
	if(!$public_r['opengetpass'] && !$silence)
	{
	...
	if(!$username||!$email)
	{
		if(!$silence)
                ...
                else return array('error'=>'EmptyGetPassword');
	}
        if(!$silence)
        {
            //验证码
        ...
        $sqlusern = mysql_real_escape_string($username);
	$email=RepPostStr($email);
	if(!chemail($email))
	{
		if(!$silence) printerror("EmailFail","history.go(-1)",1);
                return array('error'=>'EmailFail');
	}
        $ur=$empire->fetch1("select ".eReturnSelectMemberF('userid,username,email')." from ".eReturnMemberTable()." where ".egetmf('username')."='{$sqlusern}' limit 1");
	$useremail=$ur['email'];
	if(!$ur['userid']||$useremail!=$email)
	{            
		if(!$silence) printerror("ErrorGPUsername","history.go(-1)",1);
                return array('error'=>'ErrorGPUsername');
	}
	...
	//发送邮件
        if(empty($getpasstitle)|| empty($getpasstext))
        {
            $pr=$empire->fetch1("select getpasstext,getpasstitle from {$dbtbpre}enewspublic limit 1");
            if(empty($getpasstitle))$getpasstitle= $pr['getpasstitle'];
            if(empty($getpasstext))$getpasstext= $pr['getpasstext'];
        }
	...
	if(!$silence) printerror("SendGetPasswordEmailSucess",$public_r['newsurl'],1);
        return TRUE;
}
...
function DoGetPassword($add){
	...
	//--------------------------- Ucenter ---------------------------
        ...
        {
	$eur=$empire->fetch1("select ".eReturnSelectMemberF('username')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$r[id]'");
	$ucresult = uc_user_edit($eur['username'],$oldpassword,$password,'',1);
	if($ucresult == -1)
	{
...
function DoActUser($add){
	...
	$usql=$empire->query("update {$dbtbpre}enewsmemberpub set authstr='' where userid='$r[id]'");
	printerror('ActUserSuccess',$public_r['newsurl'].'e/member/login/',1);//LGM修改,转到登录页面[2014年3月14日11:05]
}
