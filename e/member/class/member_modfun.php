...
function ReturnDoMemberF($fid,$add,$mr,$ecms=0,$username='',$admin=0){
	...
	$pr=$empire->fetch1("select openmembertranimg,memberimgsize,memberimgtype,openmembertranfile,memberfilesize,memberfiletype from {$dbtbpre}enewspublic limit 1",MYSQL_ASSOC);
	$formr=$empire->fetch1("select fid,enter,mustenter,filef,imgf,canaddf,caneditf,checkboxf from {$dbtbpre}enewsmemberform where fid='$fid'",MYSQL_ASSOC);
	//检测必填字段
		...
		if(strstr($formr['filef'],",".$mf.",")||strstr($formr['imgf'],",".$mf.","))//附件
		{
			...
			if($_FILES[$mfilef]['name'])
			{
				if(strstr($formr['imgf'],",".$mf.","))//图片
				{
					...
					{
						printerror("CloseQTranPic",'history.go(-1)',1);
					}
				}
				else//附件
				{
					...
					{
						printerror("CloseQTranFile",'history.go(-1)',1);
					}
				}
			}
			elseif(!trim($add[$mf])&&!$mr[$mf])
			{
				printerror("EmptyQMustF",'history.go(-1)',1);
			}
		}
		else
		{
		...
		if(strstr($formr['filef'],",".$f.",")||strstr($formr['imgf'],",".$f.","))
		{
			...
			if($_FILES[$filetf]['name'])
			{
				...
				if(strstr($formr['imgf'],",".$f.","))//图片
				{
					...
					{
						printerror("TooBigQTranFile",'history.go(-1)',1);
					}
