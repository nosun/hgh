...
function EditGbookClass($add,$do=0,$userid,$username){
	...
	if(empty($do))
	{
		$add['checked']=(int)$add['checked'];
		$add['groupid']=(int)$add['groupid'];
		$add['rname'] = trim($add['rname']);
		$level="gbook";
		$table="{$dbtbpre}enewsgbookclass";
		$location="GbookClass.php";
		$mychecked=",checked=".$add['checked'].",groupid=".$add['groupid'].",bookurl='".$add['bookurl']."',rname='".$add['rname']."'";
	}
	else
	{
		$level="feedback";
...
/**
 * 发送站内短消息(LGM修改,加入消息体可解析[2014年3月18日23:00])
 * @global type $empire
 * @global type $dbtbpre
 * @param string $title 标题
 * @param string $msgtext 内容
 * @param datetime $msgtime 日期
 * @param int $userid
 * @param string $username
 * @param bool $silence 短消息是否(默认)静默
 * @param bool $toparse 是否需要解析
 * @param array $parseRepData 解析过程中用到的变量表
 */
function SendSiteMsg($title, $msgtext, $msgtime, $userid, $username, $silence=FALSE, $toparse = FALSE, array &$parseRepData = array())
{
    global $empire, $dbtbpre,$public_r;
    if ($toparse)
    {
        if($parseRepData==NULL) $parseRepData = array();
        if(!isset($parseRepData['sitename'])) $parseRepData['sitename'] = $public_r['sitename'];
        if(!isset($parseRepData['newsurl'])) $parseRepData['newsurl'] = $public_r['newsurl'];
        if(!isset($parseRepData['date'])) $parseRepData['date'] = date("Y-m-d");
        $msgtext = preg_replace('/\[\!\-\-(\w+)\-\-\]/',  '\$$1', $msgtext);
        $msgtext = MsgCore::EvalPhpStr($msgtext, $parseRepData);
        $title = MsgCore::EvalPhpStr($title, $parseRepData);        
    }
    $username = mysql_real_escape_string($username);
    if(empty($msgtime))$msgtime = date('Y-m-d H:i:s');
    ...
    if (!$silence)
    {
        $newhavemsg = eReturnSetHavemsg($silence, 0);
        $usql = $empire->query("update " . eReturnMemberTable() . " set " . egetmf('havemsg') . "='$newhavemsg' where " . egetmf('userid') . "='" . $userid . "' limit 1");
    }
}
/**
 * 获取站内消息的最大ID,可指定用户名(LGM增加[2014年3月19日20:51])
 * @global resource $empire
 * @global string $dbtbpre
 * @param string $username
 * @return int
 */
function GetLastSiteMsgID($username=NULL)
{
      global $empire, $dbtbpre;
      $maxids=$empire->fetch1("select max(mid) mid from {$dbtbpre}enewsqmsg".(empty($username)?'':' where to_username=\''.  mysql_real_escape_string($username) .'\''),MYSQL_NUM);
      if(empty($maxids)) return 0;
      return intval($maxids[0]);      
}
/**
 * 取站内消息的数量(LGM增加[2014年3月20日12:12])
 * @global resource $empire
 * @global string $dbtbpre
 * @param string $username
 * @param bool $haveread
 * @param int $senderuid 发送人的ID
 * @param bool $isadmin
 * @param bool $issys
 * @param string $title
 * @param datetime $msgtime string|int
 * @return int
 */
function GetSiteMsgCount($username=NULL,$haveread=NULL,$senderuid=NULL,$isadmin=NULL,$issys=NULL,$title=NULL,$msgtime=NULL)
{
    global $empire, $dbtbpre;
    $cmd = "select count(1) as count from {$dbtbpre}enewsqmsg";
    $where = array();
    if($username)  $where[] = '`to_username`=\''.mysql_real_escape_string($username).'\'';
    if($haveread!==NULL) $where[] = 'haveread='.(int)$haveread;
    if($senderuid!==NULL) $where[] = 'from_userid='.(int)$senderuid;
    if($isadmin!==NULL) $where[] = 'isadmin='.(int)((bool)$isadmin);
    if($issys!==NULL) $where[] = 'issys='.(int)((bool)$issys);
    if(!empty($title))  $where[] = '`title`=\''.mysql_real_escape_string($title).'\'';    
    if($msgtime!==NULL)
    {
        if(is_numeric($msgtime)==FALSE)   $where[] = '`msgtime`>\''.mysql_real_escape_string($msgtime).'\'';
        else   $where[] = '`msgtime`>FROM_UNIXTIME('.$msgtime.')';
    }
    if(count($where)>0)
    {
       $cmd.=' where '. implode(' AND ', $where);
    }
    $countx=$empire->fetch1($cmd,MYSQL_NUM);
    if(empty($countx)) return 0;
    return (int)$countx[0];
