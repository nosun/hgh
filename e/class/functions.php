...
function AdminReturnClassLink($classid){
	global $class_r,$fun_r,$ecmscheck;
	$addcheck='';
	if($ecmscheck)
	{
	...
	$r=explode("|",$class_r[$classid][featherclass].$classid."|");
        $adminpath2 = GetValueFromCookie('pathn',1,FALSE,'admin',NULL);
        if(DIRECTORY_SEPARATOR!='/') $adminpath2 = str_replace(DIRECTORY_SEPARATOR, '/', $adminpath2);
        $adminpath = '/e/'.$adminpath2.'/';//LGM增加,确定后台路径[2014年3月12日20:25]
	$string="<a href=\"".$adminpath."ListAllInfo.php?tbname=".$class_r[$classid][tbname].$addcheck."\">".$fun_r['AdminInfo']."</a>";
	$count=count($r)-1;
	for($i=1;$i<$count;$i++)
	{
		$curl=$class_r[$r[$i]][islast]?"ListNews.php?classid=".$r[$i].$addcheck:"ListAllInfo.php?tbname=".$class_r[$r[$i]][tbname]."&classid=".$r[$i].$addcheck;
		$string.="&nbsp;>&nbsp;<a href=\"".$adminpath."$curl\">".$class_r[$r[$i]][classname]."</a>";
    }
	return $string;
}
...
function AddInfoToZt($ztid,$zcid,$classid,$id,$newstime,$isgood=0,$ecms=0){
	...
	$r=$empire->fetch1("select isgood from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id=".$id);
	$isgood=$r[isgood];
	if($ecms==1)//检查是否增加过
	{
...
function AddMoreInfoToZt($ztid,$zcid,$tbname,$where,$ecms=0){
	...
	$sql=$empire->query("select id,classid,newstime,isgood from {$dbtbpre}ecms_".$tbname.($ecms==0?'':'_index')." where ".$where);
	while($r=$empire->fetch($sql))
	{
		...
		else
		{
			...
			$empire->query("insert into {$dbtbpre}enewsztinfo(ztid,cid,classid,id,newstime,mid,isgood) values('$ztid','$zcid','$r[classid]','$r[id]','$r[newstime]','$mid','$r[isgood]]');");
		}
	}
}
...
function NewsBq($classid,$indextext,$enews=0,$doing=0){
	...
	$classlevel='';
        $pagetitle = '';//LGM添加[2014年3月16日20:57]
	if($enews==0)//生成大栏目
	{
	...
	//写文件
	$r = WriteFiletext($file,AddCheckViewTempCode().$indextext);
        $retinfo = array();
        if($r<1)
            $retinfo['error'] = array($file.'[{'.$pagetitle.'}页面中间代码]写入失败!'); //LGM 增加,返回错误信息[2014年3月16日19:27]
	//读取文件内容
        ob_start();
	include($file);
	$string=ob_get_contents();
	...
	$r = WriteFiletext($truefile,$classlevel.$string);
        if($r<1)//LGM 增加,返回错误信息[2014年3月16日19:27]
         {            
             if(!isset($retinfo['error']))$retinfo['error'] = array();
             $retinfo['error'] =array($truefile.'[{'.$pagetitle.'}页面生成代码]写入失败!'); 
         }
         if(!empty($retinfo)) return  $retinfo;
	return $string;
}
...
function GetPlTempPage($pltempid=0){
	...
	while($ptr=$empire->fetch($ptsql))
	{
		...
		$plfiletemp=str_replace("<!--empire.listtemp.top-->",$listtemp_r[0],$plfiletemp);
                $plfiletemp=str_replace("<!--empire.listtemp.center-->",$listtemp_r[2],$plfiletemp);
		$plfiletemp=str_replace("<!--empire.listtemp.footer-->",$listtemp_r[4],$plfiletemp);
                
		//最热列表
		$listtemp_hotest=str_replace("[!--plid--]","<?=\$r[plid]?>",$listtemp_r[1]);
		$listtemp_hotest=str_replace("[!--pltext--]","<?=\$saytext?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--pltime--]","<?=\$saytime?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--plip--]","<?=\$sayip?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--username--]","<?=\$plusername?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--userid--]","<?=\$r[userid]?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--includelink--]","<?=\$includelink?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--zcnum--]","<?=\$r[zcnum]?>",$listtemp_hotest);
		$listtemp_hotest=str_replace("[!--fdnum--]","<?=\$r[fdnum]?>",$listtemp_hotest);
		$listtemp_hotest=ReplacePlListVars($listtemp_hotest,$r,$pr,0);
		$plfiletemp=str_replace("<!--empire.listtemp.hotest-->",$listtemp_hotest,$plfiletemp);
                
		//最新列表
		$listtemp_latest=str_replace("[!--plid--]","<?=\$r[plid]?>",$listtemp_r[3]);
		$listtemp_latest=str_replace("[!--pltext--]","<?=\$saytext?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--pltime--]","<?=\$saytime?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--plip--]","<?=\$sayip?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--username--]","<?=\$plusername?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--userid--]","<?=\$r[userid]?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--includelink--]","<?=\$includelink?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--zcnum--]","<?=\$r[zcnum]?>",$listtemp_latest);
		$listtemp_latest=str_replace("[!--fdnum--]","<?=\$r[fdnum]?>",$listtemp_latest);
		$listtemp_latest=ReplacePlListVars($listtemp_latest,$r,$pr,0);
		$plfiletemp=str_replace("<!--empire.listtemp.latest-->",$listtemp_latest,$plfiletemp);
...
function GetPlJsPage(){
	...
	$pltemp=str_replace("<!--empire.listtemp.center-->",$listtemp_center,$pltemp);
        $r=WriteFiletext_n($pl_filename,$pltemp);
        if($r<1)//LGM 增加,返回错误信息[2014年3月16日19:27]
        {
            $retinfo['error'] = array($pl_filename.'[评论JS模板代码]写入失败!'); 
            return $retinfo;
        }
        return TRUE;
}
...
/**
 * 去除PHP标签内的反斜扛(LGM 增加 2014年2月18日14:25)
 * @param string $tagcontent 含有php标签的代码
 */
function DecodePHPTag($tagcontent)
{
	return preg_replace_callback('/<\?(php)?\b(.+?)\?>/is', function($m){
		return '<?php '.stripSlashes($m[2]).' ?>';
	}, $tagcontent);
}
//更新登陆状态模板
function ReLoginIframe(){
	...
	$retinfo = array();$login = DecodePHPTag($login);	$loginin = DecodePHPTag($loginin);//LGM 增加 2014年2月18日14:25 
	$text=str_replace("<!--login-->",$login,$loginiframetemp);
	$text=str_replace("<!--loginin-->",$loginin,$text);
	$file="../member/iframe/index.php";        
	$r=WriteFiletext_n($file,$text);
        if($r<1) $retinfo['error'] = array($file.'[框架登陆状态调用代码]写入失败!'); //LGM 增加,返回错误信息[2014年3月16日19:27]
	//JS登陆状态调用
	...
	$login="document.write(\"".addslashes(stripSlashes(str_replace("\r\n","",$r[0])))."\");";
	$loginin="document.write(\"".addslashes(stripSlashes(str_replace("\r\n","",$r[1])))."\");";
	$login = DecodePHPTag($login);$loginin = DecodePHPTag($loginin);//LGM 增加 2014年2月18日14:25 
	$text=str_replace("<!--login-->",$login,$loginiframetemp);	
	$text=str_replace("<!--loginin-->",$loginin,$text);
	...
	$r=WriteFiletext_n($file,$text);
         ...
         {
             if(!isset($retinfo['error']))$retinfo['error'] = array();
             $retinfo['error'][] = $file.'[JS登陆状态调用代码]写入失败!'; 
         }
         if(!empty($retinfo)) return  $retinfo;
         return TRUE;
}
...
function GetDownloadPage(){
	...
	$r = WriteFiletext($file,AddCheckViewTempCode().$temptext);
        if($r<1)//LGM 增加,返回错误信息[2014年3月16日19:27]
        {
            $retinfo['error'] = array($file.'[下载页面模板代码]写入失败!'); 
            return $retinfo;
        }
        return TRUE;
}
...
function is_login($uid=0,$uname='',$urnd=''){
	global $empire,$public_r,$dbtbpre;
	$userid=$uid?$uid:getcvar('loginuserid',1,TRUE);//LGM修改,加密方式[2014年3月17日14:39]
...
function DoECookieRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime){
	...
	{
		if(!DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval))//LGM修改,判断是否写入成功[2014年3月17日10:00]
                {
                    $p = array('msg'=>'文件认证写入失败');
                    printerror('FileOperateFail',$p);
                }
	}
...
function DelECookieRnd(){
	delcookie('loginecmsckpass',1);
        delcookie('pathn',1);//LGM 添加,删除后台路径名[2014年3月12日18:25]
}
...
function DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	...
	$file=ECMS_PATH.'e/data/adminlogin/user'.$userid.'_'.md5(md5($username.'-empirecms!check.file'.$truelogintime.'-'.$rnd.$ecms_config['esafe']['ecookiernd']).'-'.$ip.'-'.$userid.'-'.$rnd.$adminstyle.'-'.$groupid.'-'.$dbdata.$sessval).'.log';
        return (bool)WriteFiletext_n($file,'EmpireCMS');//LGM修改,返回是否已经执行成功[2014年3月17日9:32]
}
...
function DoECreatAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	...
	$empire->query("replace into {$dbtbpre}enewsuserloginck(userid,andauth) values('$userid','$andauth');");
        return $empire->affectnum();//LGM修改,返回是否已经执行成功[2014年3月17日9:32]
}
...
function DoEDelAndAuthRnd($userid){
	...
	$empire->query("delete from {$dbtbpre}enewsuserloginck where userid='$userid'");
        return $empire->affectnum();//LGM修改,返回受影响的行数[2014年3月17日11:55]
}
...
function CheckFtpConnect($ftphost,$ftpport,$ftpusername,$ftppassword,$ftppath,$ftpssl=0,$pasv=0,$tranmode=0,$timeout=0){
	...
	$result=$eftp->fconnect($ftphost,$ftpport,$ftpusername,$ftppassword,$ftppath,$ftpssl,$pasv,$tranmode,$timeout,1);
        file_put_contents('/tmp/1.log', $result);	
        if($result=='HostFail')
	{
...
function ChMustAddF($mid,$f,$value){
	...
	if(strstr($emod_r[$mid]['mustqenterf'],','.$f.','))
	{
		if(!trim($value))
		{
			$metadata = array('FieldName'=>ChGetFname($mid,$f),'Field'=>$f);//LGM修改 [2014年2月25日10:05]
			printerror("EmptyMustF","history.go(-1)",$metadata);
		}
	}
}
...
function ChIsOnlyAddF($mid,$id,$f,$value,$isq=0){
	...
	if(strstr($emod_r[$mid]['onlyf'],','.$f.','))
	{
		...
		if($num)
		{
			$meta = array('FieldName'=>ChGetFname($mid,$f));//LGM修改[2014年2月25日10:40]
			if($isq==1)
			{
				printerror("ReIsOnlyF","history.go(-1)",1,$meta);
			}
			else
			{
				printerror("ReIsOnlyF","history.go(-1)",$meta);
			}
		}
	}
}
...
function ReturnAddF($add,$modid,$userid,$username,$do=0,$rdata=0,$ch=0){
	...
	if(empty($do))//增加
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			{
				ChMustAddF($modid,$f,$value);//检测必填字段
			...
			if($f=="newstext")
			{
				...
				if($add['autopage']&&!strstr($value,"[!--empirenews.page--]"))
				{
					...
					{$add['autosize']=AUTOPAGESIZE;}//LGM修改成常量,2014年2月25日20:31
	...
	elseif($do==1)//修改
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			if($f=="special.field"||!strstr($emod_r[$modid]['caneditf'],','.$f.','))
			{
                        ...
                        if(!isset($add[$f]) && $ch==0)//如果没有设置设,又不须要检查的情况下,不处理(LGM 修改[2014年3月4日8:46])
                            continue;
			$add[$f]=ReturnCheckboxAddF($add[$f],$modid,$f);//复选框
			$add[$f]=ReturnMoreValueAddF($add,$add[$f],$modid,$f,$do);//多值
			$value=RepPhpAspJspcodeText($add[$f]);
	...
	elseif($do==8)//同步修改
	{
		...
		{
			$f=$r[$i];
			if($f=='special.field' || (($f=="newstext" || $f=="smalltext")  && !isset($add[$f])))//LGM 修改,如果概述或者内容没有设置,则不记入语句[2014年3月3日17:24]
			{
				continue;
			}
			$value=$add[$f];
...
function GetConfig($domod=0){
	...
	$r = WriteFiletext_n($filename,$setting);
        if($r<1)//LGM 增加,返回错误信息[2014年3月16日19:27]
        {
            $retinfo['error'] = array($filename.'[配置文件代码]写入失败!'); 
            return $retinfo;
        }
        return TRUE;
}
