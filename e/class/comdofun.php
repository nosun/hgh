...
function DoRepNewstext($start,$oldword,$newword,$field,$classid,$tid,$tbname,$over,$dozz,$dotxt,$userid,$username){
	...
	elseif($fr['savetxt'])
	{
		...
		$start=(int)$start;
		$b=0;
		$sql=$empire->query("select id,checked from {$dbtbpre}ecms_".$tbname."_index where id>$start".$add1." and isurl=0 order by id limit ".$public_r[dorepnum]);
		while($r=$empire->fetch($sql))
		{
			$b=1;
			$newstart=$r[id];
...
function ClearBreakInfo($add,$userid,$username){
	...
	$meta = array('cleanCount'=>$affnum);
	printerror("ClearBreakInfoSuccess","",$meta);//LGM 修改[2014年2月25日10:48]
}
