...
function AddFileDataTable($add,$userid,$username){
	global $empire,$dbtbpre;
	//操作权限
	CheckLevel($userid,$username,$classid,"filetable");
	$datatb=(int)$add['datatb'];
	if(!$datatb)
	{
		printerror("EmptyFileDataTable","history.go(-1)");
	}
	$tr=$empire->fetch1("select filedatatbs from {$dbtbpre}enewspublic limit 1");
	if(strstr($tr['filedatatbs'],','.$datatb.','))
	{
		printerror("ReFileDataTable","history.go(-1)");
	}
	if(empty($tr['filedatatbs']))
	{
		$tr['filedatatbs']=',';
	}
	$newdatatbs=$tr['filedatatbs'].$datatb.',';
	//建表
	$odtb=$dbtbpre."enewsfile_1";
	$dtb=$dbtbpre."enewsfile_".$datatb;
	CopyEcmsTb($odtb,$dtb);
	$sql=$empire->query("update {$dbtbpre}enewspublic set filedatatbs='$newdatatbs' limit 1");
	GetConfig();//更新缓存
	if($sql)
	{
		//操作日志
		insert_dolog("datatb=$datatb");
		printerror("AddFileDataTableSuccess","file/ListFileDataTable.php");
	}
	else
	{
		printerror("DbError","history.go(-1)");
...
function DefFileDataTable($add,$userid,$username){
	...
	if(!$datatb)
	{
		printerror("NotChangeFileDataTable","history.go(-1)");
	}
	$tr=$empire->fetch1("select filedatatbs from {$dbtbpre}enewspublic limit 1");
	if(!strstr($tr['filedatatbs'],','.$datatb.','))
	{
	...
	$sql=$empire->query("update {$dbtbpre}enewspublic set filedeftb='$datatb' limit 1");
	GetConfig();//更新缓存
	if($sql)
	{
		//操作日志
		insert_dolog("datatb=$datatb");
		printerror("DefFileDataTableSuccess","file/ListFileDataTable.php");
	}
	else
	{
		printerror("DbError","history.go(-1)");
...
function DelFileDataTable($add,$userid,$username){
	...
	$tr=$empire->fetch1("select filedatatbs,filedeftb from {$dbtbpre}enewspublic limit 1");
	if(!strstr($tr['filedatatbs'],','.$datatb.','))
	{
		printerror("NotChangeFileDataTable","history.go(-1)");
	}
	if($tr['filedeftb']==$datatb||$datatb==1)
	{
		printerror("NotDelDefFileDataTable","history.go(-1)");
	}
	$newdatatbs=str_replace(','.$datatb.',',',',$tr['filedatatbs']);
	$sql=$empire->query("update {$dbtbpre}enewspublic set filedatatbs='$newdatatbs' limit 1");
	//删除附件
	$filesql=$empire->query("select filename,path,classid,fpath from {$dbtbpre}enewsfile_{$datatb}");
	while($filer=$empire->fetch($filesql))
	{
		DoDelFile($filer);
	}
	//删除表
	$deltb=$empire->query("DROP TABLE IF EXISTS {$dbtbpre}enewsfile_".$datatb.";");
	GetConfig();//更新缓存
	if($sql)
	{
		//操作日志
		insert_dolog("datatb=$datatb");
		printerror("DelFileDataTableSuccess","file/ListFileDataTable.php");
	}
	else
	{
		printerror("DbError","history.go(-1)");
