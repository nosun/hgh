...
define('PV_EINAME','EITypes');//第三方交互扩展变量键名  LGM ADD 2014年2月12日15:19
define('AUTOPAGESIZE',5000);//自动分页每页字数  LGM ADD 2014年2月25日20:28
...
require_once (ECMS_PATH.'e'.DIRECTORY_SEPARATOR.'class'.DIRECTORY_SEPARATOR.'MsgCore.php');//加入消息处理类
//TODO --------------- 公共 ---------------

/**
 * 设置COOKIE (LGM修改 增加解密选项[2014年3月11日18:00])
 * @global array $ecms_config
 * @param string $name COOKIE键名
 * @param string $val 要设置的COOKIE值
 * @param int $life 有效期,Unix timestamp 或秒 ,0为Seesion(默认)
 * @param int $ecms 1为后台,0为前台(默认),-1原生Cookie,不添加前缀
 * @param bool $isEncrypt 是否加密(如果加密,默认在前台无法用JS读取)
 * @param string $key 密钥(只能用ASCII字符)
 * @param int $kstyle 加密优先方式,0为Mcrypt方式(默认),1为自定义的混淆法
 * @param bool $secure 是/否(默认)只用于HTTPS,443端口
 * @param bool $httponly 是/否(默认)只能被HTTP访问,JS不能读取,为NULL时未定义,由程序决定
 * @return bool
 */
function esetcookie($name,$val,$life=0,$ecms=0,$isEncrypt=FALSE,$key=NULL,$kstyle=0,$secure = false,$httponly = NULL){
	global $ecms_config;
        ...
        if($cpre>-1)
        {
            if($ecms==0) $cpre = $ecms_config['cks']['ckvarpre'];
            elseif($ecms==1) $cpre = $ecms_config['cks']['ckadminvarpre']; 
        }
        //如果需要加密:
        if(!!$isEncrypt && !empty($val) && method_exists('MsgCore','Encrypt'))
        {
            $val = MsgCore::Encrypt($val, $key, $kstyle);
            if($httponly===NULL)  $httponly = TRUE;//加密后不允许在前台用JS读取
        }
        if($httponly===NULL)  $httponly = FALSE;//$httponly=TRUE 已经加密的Cookie在客户端不能被JS访问
	return setcookie($cpre.$name,$val,$life,$ecms_config['cks']['ckpath'],$ecms_config['cks']['ckdomain'],$secure,$httponly);
}


/**
 * 获取指定键名的cookie值 (LGM修改 增加加密选项[2014年3月11日18:00])
 * @global array $ecms_config
 * @param string $var COOKIE键名
 * @param int $ecms 1为后台,0为前台(默认),-1原生Cookie,不添加前缀
 * @param bool $isEncrypt 是/否(默认)加密(如果为是,则强制性使用解密方式,解密失败则返回空,这是与GetValueFromCookie的区别)
 * @param string $key 密钥(只能用ASCII字符)
 * @param int $kstyle 加密优先方式,0为Mcrypt方式(默认),1为自定义的混淆法
 * @return string
 */
function getcvar($name,$ecms=0,$isEncrypt=FALSE,$key=NULL,$kstyle=0){
	global $ecms_config;
        if(empty($name)) return FALSE;
        $cpre = '';
        $ecms = (int)$ecms;
	...
	$tvar=$cpre.$name;
        $v = NULL;
        if(array_key_exists($tvar, $_COOKIE)) $v = $_COOKIE[$tvar];
        if(!!$isEncrypt && !empty($v) && method_exists('MsgCore','Encrypt'))//如果已经加密
        {
            $v = MsgCore::Decrypt($v, $key, $kstyle);                    
        }
	return $v;
}
/**
 * 删除指定键名的COOKIE (LGM 添加[2014年3月12日8:50])
 * @param string $key 要删除的COOKIE 键
 * @param int $ecms  1为后台,0为前台(默认),-1原生Cookie,不添加前缀,255为全部
 * @return bool
 */
function delcookie($key, $ecms = 0) {
    global $ecms_config;
    ...
    if($ecms==255)
    {
        if(isset($_COOKIE[$ecms_config['cks']['ckvarpre'].$key])) esetcookie($key,'',1,0); 
        if(isset($_COOKIE[$ecms_config['cks']['ckadminvarpre'].$key])) esetcookie($key,'',1,1); 
        if(isset($_COOKIE[$key])) esetcookie($key, '',1,-1);
        return TRUE;
    }
    $ecmsname = $ecms_config['cks']['ckvarpre'];
    if($ecms==1) $ecmsname = $ecms_config['cks']['ckadminvarpre'];
    elseif($ecms==-1) $ecmsname = '';
    if(isset($_COOKIE[$ecmsname.$key])) 
       return esetcookie($key, '',1,$ecms); 
    return FALSE;
}

/**
 * 取所有模型的表名(LGM 增加[2014年2月24日20:16])
 * @return array
*/
function GetModesTableName()
{
	global $dbtbpre,$emod_r;
	$tablepn= 'ecms_';
	$r = array();
	foreach ($emod_r as $d)
	{
		$r[]=$dbtbpre.$tablepn.$d['tbname'];
	}
	return $r;
}
/**
 * 从COOKIE中取出指定键的值(可能加密)
 * @author LGM[2014年3月13日21:57]
 * @param string $key COOKIE键名,必须指定
 * @param int $ecms 1为后台、0为前台(默认),-1为不加COOKIE前缀 
 * @param bool $delthecookie 是否删除COOKIE键,默认为是 
 * @param string $defaultvalue 如果为空,则用此默认值返回
 * @param array $ClearNames,被替换成空的名字列表,注:必须为小写
 * @return string | NULL
 */
function GetValueFromCookie($key, $ecms = 0, $delthecookie = TRUE, $defaultvalue = NULL,$ClearNames = array('none','empty','error')) 
  {
    $returnurl = $gotourl = getcvar($key, $ecms,FALSE);
    $enystate = MsgCore::IsEncrypt($gotourl);
    switch ($enystate) {
        case MsgCore::ES_NOTENCRYPT:
            break;
        case MsgCore::ES_ENCRYPTED:
            $gotourl = MsgCore::Decrypt($gotourl, NULL, 0, FALSE);
            break;
        case MsgCore::ES_ENCRYPTCHKERROR:
            $gotourl = NULL;
            break;
        default:
            $gotourl = NULL;
    ...
    if ($gotourl != NULL && ( ((!empty($ClearNames)) && in_array(strtolower($gotourl), $ClearNames)==TRUE) || strlen(trim($gotourl)) == 0) ) {
        $gotourl = NULL;
    }
    if ($delthecookie) delcookie($key, $ecms); //删除    
    if (empty($gotourl)) {
        $gotourl = $defaultvalue;
    }
    return $gotourl;
}

/**
 * 提示信息函数(LGM修改[2014年2月23日16:49]，处理了信息头,当设置了$public_r['ScreenSilent']时，表示静默方式)
 * @param string $error 提示信息内容，可以为语言包变量，也可以直接提示内容。
 * @param string $gotourl 提示信息转向链接地址 (不设置将会读取COOKIE[returnurl])
 * @param number $ecms 0为后台提示信息(默认)、8为后台提示信息(alert方式)、9为前台提示信息(alert方式) ,7为前台弹出对话框并关闭窗口,6为后台弹出对话框并关闭窗口、其它为前台提示信息
 * @param number $noautourl 0为自动转向页面，1为不自动转向页面。此参数可省略，默认为0 
 * @param number $novar 0为提示信息内容采用语言包变量，1为提示信息内容为直接提示内容。此参数可以省略，默认为0 
 * @param array $metadata 元数据(可以指明信息类型[IH_InfoType]与错误级别[IH_ErrorLevel],[waittime]对话框显示时间,[ScreenSilent]是否强制在屏幕{0}/字符串{1}输出,优先级高于MsgCore::ScreenSilent)
 * @return string | NULL  返回如：{:E,20,{"url":"http:\/\/www.baidu.com\/","active":"exit"}:}您来自的链接不存在
*/
function printerror($error="",$gotourl="",$ecms=0,$noautourl=0,$novar=0,array $metadata=array()){
	global $empire,$public_r,$ecms_config,$enews_r,$message_r,$qmessage_r,$fun_r;
	//LGM 修改[2014年2月23日13:40]
	$argc= func_num_args();
	if($argc>0)
	{
		$lastarg=func_get_arg($argc-1);//取最后一个参数
		if(is_array($lastarg))
		{
			switch($argc)
			{
				case 1:$error="";break;
				case 2:$gotourl="";break;
				case 3:$ecms=0;break;
				case 4:$noautourl=0;break;
				case 5:$novar=0;break;
				default:break;
			}
			if(isset($lastarg['error'])){$error=$lastarg['error'];unset($lastarg['error']);}
			if(isset($lastarg['gotourl'])){$gotourl=$lastarg['gotourl'];unset($lastarg['gotourl']);}
			if(isset($lastarg['ecms'])){$ecms=$lastarg['ecms'];unset($lastarg['ecms']);}
			if(isset($lastarg['noautourl'])){$noautourl=$lastarg['noautourl'];unset($lastarg['noautourl']);}
			if(isset($lastarg['novar'])){$novar=$lastarg['novar'];unset($lastarg['novar']);}
			if(isset($lastarg['metadata']))
			{
				if(empty($metadata))$metadata=$lastarg['metadata'];
				unset($lastarg['metadata']);
			}
			else 
			{
				if(empty($metadata))
				{
					$metadata = array();
					foreach ($lastarg as $k=>$v)  if(!empty($v))$metadata[$k]=$v;					
				}
				if(empty($metadata)) $metadata = NULL;
			}
		}
	}
	$ScreenSilent = MsgCore::ScreenSilent();        
        if(isset($metadata['ScreenSilent'])) $ScreenSilent = (bool)$metadata['ScreenSilent'];
        $waittime = 2;
        if(isset($metadata['waittime'])) $waittime = intval($metadata['waittime']);
        if(empty($gotourl))
        {
            $gotourl = getcvar('returnurl',$ecms);
            $enystate = MsgCore::IsEncrypt($gotourl);
            switch ($enystate)
            {
                case MsgCore::ES_NOTENCRYPT:
            ...
            if($gotourl!=NULL &&  (strcasecmp('none',$gotourl)==0 || strcasecmp('empty',$gotourl)==0 || strlen(trim($gotourl))==0))
            {
                  $gotourl = NULL;  
                  delcookie('returnurl',$ecms); //删除
            }
            if(!empty($gotourl))
            {               
                delcookie('returnurl',$ecms); //删除
            ...
            {
               $gotourl = getcvar('returnurl',-1);
               if(!empty($gotourl)) 
                   delcookie('returnurl',-1);
               else
                 $gotourl = $_REQUEST['returnurl']; 
               if(empty($gotourl) || strcasecmp($gotourl,'none')==0 || strcasecmp($gotourl,'empty')==0)
               {
	...
	if($ecms==9)//前台弹出对话框
	{
		@include_once AbsLoadLang('pub/q_message.php');		
		$error=empty($novar)?MsgCore::TranslatMsg($qmessage_r, $error):$error;
		$pr2 =  MsgCore::ProcessErrorInfo($error,$metadata,$ScreenSilent);
		if(!$ScreenSilent)
			MsgCore::AlertMsgExit("alert('".$pr2['info']."');".$gotourl_js);		
		else 
			throw new PutInfoException($pr2['info']);
	}
	elseif($ecms==8)//后台弹出对话框
	{
		@include_once AbsLoadLang('pub/message.php');		
		$error=empty($novar)?MsgCore::TranslatMsg($message_r, $error):$error;
		$pr2 = MsgCore::ProcessErrorInfo($error,$metadata,$ScreenSilent);
		if(!$ScreenSilent)
			MsgCore::AlertMsgExit("alert('".$pr2['info']."');".$gotourl_js);
		else
	...
	elseif($ecms==7)//前台弹出对话框并关闭窗口
	{
		...
		$pr2 = MsgCore::ProcessErrorInfo($error,$metadata,$ScreenSilent);
		if(!$ScreenSilent)
			MsgCore::AlertMsgExit("alert('".$pr2['info']."');window.close();");
		else
			throw new PutInfoException($pr2['info']);
	}
	...
	elseif($ecms==0)
	{
		$kcf= AbsLoadLang('pub/message.php');		
		@include_once($kcf);
		$error=empty($novar)?MsgCore::TranslatMsg($message_r, $error):$error;   
 		$pr2 = MsgCore::ProcessErrorInfo($error,$metadata,$ScreenSilent);  
		if(!$ScreenSilent)
		{
                    $error = $pr2['info']; 
                    $adminpath = GetValueFromCookie('pathn',1,FALSE,'admin',NULL); 
                    if(DIRECTORY_SEPARATOR!='/') $adminpath = str_replace('/', DIRECTORY_SEPARATOR, $adminpath);
                    $mypath = ECMS_PATH.'e'.DIRECTORY_SEPARATOR.$adminpath.DIRECTORY_SEPARATOR.'message.php';   
                    if(file_exists($mypath))  @include($mypath);		
		}
		else 
			throw new PutInfoException($pr2['info']);
	}
	...
	{//前台提示
		...
		if(!$ScreenSilent)
                {
                    $error = $pr2['info'];
                    $mypath = ECMS_PATH.'e'.DIRECTORY_SEPARATOR.'message'.DIRECTORY_SEPARATOR.'index.php';
                    if(file_exists($mypath))  @include($mypath);       
                }
		else 
			throw new PutInfoException($pr2['info']);
...
/**
 * 直接文字提示信息函数 (LGM修改[2014年2月23日16:49]，处理了信息头,当设置了$public_r['ScreenSilent']时，表示静默方式)
 * @param string $error 提示信息内容
 * @param string $gotourl 提示信息转向链接地址 (不设置将会读取COOKIE[returnurl])
 * @param number $ecms 0为前台提示信息(默认)、8为后台提示信息(alert方式)、9为前台提示信息(alert方式) ,7为弹出对话框并关闭窗口
 * @param number $noautourl 0为自动转向页面，1为不自动转向页面。此参数可省略，默认为0 
 * @return string | NULL   返回如：{:N,0}:}您来自的链接不存在
*/
function printerror2($error='',$gotourl='',$ecms=0,$noautourl=0){
	global $empire,$public_r,$message_r,$qmessage_r,$fun_r;
	//LGM 修改 [2014年2月23日17:25]
	$ScreenSilent = MsgCore::ScreenSilent();
	$taxx = array();
	$pr2 = MsgCore::ProcessErrorInfo($error,$taxx,$ScreenSilent);
	//LGM 修改----------------------------------------------------
        if(empty($gotourl))
        {
            $gotourl = getcvar('returnurl',$ecms);
...
/**
 * ajax错误提示(LGM修改[2014年2月23日16:49]，处理了信息头,当设置了$public_r['ScreenSilent']时，表示静默方式)
 * @param string $result
 * @param string $ajaxarea
 * @param string $error 提示信息内容，可以为语言包变量，也可以直接提示内容。
 * @param number $ecms 1为后台提示信息、0为前台提示信息(默认)、8为后台提示信息(alert方式)、9为前台提示信息(alert方式) 
 * @param number $novar 0为提示信息内容采用语言包变量，1为提示信息内容为直接提示内容。此参数可以省略，默认为0 
 * @param array $metadata 元数据 (可以指明信息类型[IH_InfoType]与错误级别[IH_ErrorLevel],[ScreenSilent]是否强制在屏幕{0}/字符串{1}输出,优先级高于MsgCore::ScreenSilent)
 * @return string | NULL  返回如：{:E,20,{"url":"http:\/\/www.baidu.com\/","active":"exit"}:}您来自的链接不存在
*/
function ajax_printerror($result='',$ajaxarea='ajaxarea',$error='',$ecms=0,$novar=0,array $metadata=array()){
	global $empire,$public_r,$ecms_config,$message_r,$qmessage_r,$fun_r;
	//LGM 修改[2014年2月23日13:40]
	$argc= func_num_args();
	if($argc>0)
	{
		...
		if(is_array($lastarg))
		{
			...
			{
				case 1:$result="";break;
				case 2:$ajaxarea="ajaxarea";break;
				case 3:$error="";break;
				case 4:$ecms=0;break;
				case 5:$novar=0;break;
				default:break;
			}
			if(isset($lastarg['result'])){$result=$lastarg['result'];unset($lastarg['result']);}
			if(isset($lastarg['ajaxarea'])){$ajaxarea=$lastarg['ajaxarea'];unset($lastarg['ajaxarea']);}
			if(isset($lastarg['ecms'])){$ecms=$lastarg['ecms'];unset($lastarg['ecms']);}
			if(isset($lastarg['error'])){$error=$lastarg['error'];unset($lastarg['error']);}
			if(isset($lastarg['novar'])){$novar=$lastarg['novar'];unset($lastarg['novar']);}
			if(isset($lastarg['metadata']))
	...
	if(!empty($pr2)) $infoheader = '|'.$pr2['head'];
	$string=$result.'|'.$ajaxarea.'|'.($pr2==NULL?$error :$pr2['info']).$infoheader;
	echo $string;
	db_close();
	$empire=null;
	exit();
...
/**
 * 直接转向 (LGM修改,当转向地址为NULL时,读取COOKIE内的returnurl[2014年3月13日15:00])
 * @global object $empire 数据库对象
 * @global array $public_r
 * @global array $ecms_config
 * @param string $gotourl 要转向的地址
 * @param string $error
 * @param int $sec 等待秒数
 * @param int $ecms 0:前台(默认),1-后台
 */
function printerrortourl($gotourl = '', $error = '', $sec = 0,$ecms=0) 
   {
    global $empire, $link;
    if ($gotourl === NULL) {
        $gotourl = GetValueFromCookie('returnurl',$ecms,TRUE);
        if(empty($gotourl))
        {
           $gotourl = GetValueFromCookie('returnurl',-1,TRUE,'');
        }
    }
...
/**
 * 获取语言文件的绝对路径(LGM 增加[2014年2月24日15:29])
 * @param string $file
 * @return string
*/
function AbsLoadLang($file){
	global $ecms_config;
        if(DIRECTORY_SEPARATOR != '/') $file = str_replace('/', DIRECTORY_SEPARATOR, $file);
	return ECMS_PATH.'e'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'language'.DIRECTORY_SEPARATOR.$ecms_config['sets']['elang'].DIRECTORY_SEPARATOR.$file;
}
...
/**
 * 写文件,用stripSlashes处理(LGM 修改,有返回值[2014年3月16日19:12])
 * @global array $public_r
 * @param string $filepath 文件路径 
 * @param string $string 写入之内容
 * @return int | FALSE 被写入的字节数
 */
...
/**
 * 写文件(LGM 修改,有返回值[2014年3月16日19:12])
 * @global array $public_r
 * @param string $filepath 文件路径 
 * @param string $string 写入之内容
 * @return int | FALSE 被写入的字节数
...
function FWEmptyPassword(){
	global $ecms_config;
	delcookie($ecms_config['fw']['adminckpassvar'],1);
}
/**
 * 格式化微秒时间
 * @param string $microtime 微秒 UNIX时间戳
 * @return string
 */
 ...
 * 调试变量
 * @param mixed  $v
 * @param string $info 信息上下文,用{$v}来替换前面的参数$v,如果不含变量,则添加在最前面
 * @param boolean $stop
 * @param int $deep
 */
function DebugVar($v,$info,$stop=FALSE,$deep=0){
	$btrace= debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
	$ix=$btrace[$deep]['file'].'['.strval(intval($btrace[$deep]['line'])-1).']';
	if(isset($btrace[$deep+1]))
	{
		$trg='';
		if(isset($btrace[$deep+1]['class']))
			$trg=$btrace[$deep+1]['class'].'::';
		if(isset($btrace[$deep+1]['function']))
			$trg.=$btrace[$deep+1]['function'];
		$ix.=$trg;
	}
        $q = '';//前
        $h = '';//后
	...
	echo '<pre>'.format_mt(microtime()).'('.$ix.')#:'.$q.PHP_EOL;
 ...
 * @param mixed $v 变量
 * @param string $info 信息上下文,用{$v}来替换前面的参数$v,如果不含变量,则添加在最前面
 * @param string $logPath 日志文件的路径 
 * @param int $deep 函数跟踪深度
 */
function DebugVarF($v,$info,$logPath=NULL,$deep=0){
        static $dvfindex=0;
        $dvfindex++;
	$btrace= debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
	$ix=$btrace[$deep]['file'].'['.strval(intval($btrace[$deep]['line'])-1).']';
	if(isset($btrace[$deep+1]))
	{
        ...
        if(empty($logPath)) $logPath = ECMS_PATH.'e/data/tmp/log.txt';
        $q = '';//前
        $h = '';//后
        if(!empty($info))
        {
        ...
        ob_start();
	echo strval($dvfindex).'>'.format_mt(microtime()).'('.$ix.')#:'.$q.PHP_EOL;
	if(is_array($v))print_r($v);
	else var_dump($v);
	echo ' '.$h.PHP_EOL.'-----------------------------------'.PHP_EOL;
 ...
 * @param mixed $value
 * @param mixed $RefType 参考类型
 * @param bool $must 是否为必须
 * @return array 传入值$params的引用 
 */
function BuildUpdatePA(array &$params,$fieldname,$value,$RefType='string',$must=FALSE)
{
    if(empty($fieldname) || $params===NULL ) return;
    if(is_numeric($RefType) && $value!==NULL)
    {
        if(is_int($RefType))
            $value = intval($value);
        else {
    ...
    $a =$params[$fieldname] = array(
        'v'=> $value   
    );
    if($must)
        $params[$fieldname]['m']=TRUE;
}
/**
 * 构造简单的Update SQL语句(LGM 增加 2014年3月3日16:20)
 * @param string $setpre set 之前的语句
 * @param array $setable set 表,包含键值v(值),m(必须) 
 * @param string $wheresuf where 之后的语句
 * @return string
 */
function SimpleJoinUpdateSQL($setpre,array &$setable,$wheresuf,$othersets=''){
    if(empty($setable))return '';
    if(array_depth($setable)!=2) return '';
    $cmd=strval($setpre).' ';
    foreach ($setable as $k=>$v)
    {
        $c='';
        if(isset($v['v']))
        {            
            ...
            {
                if(!empty($v['v']))
                        $c='`'.$k.'`=\''.mysql_real_escape_string($v['v']).'\'';
                else
                {
                    if($v['m'])
                    {
    ...
    if(empty($othersets))   $cmd=substr($cmd, 0, -1);
    else {
        $othersets = ltrim($othersets,' ,');
        $cmd.=$othersets;
    }
    $cmd.=' '.$wheresuf;
    return $cmd;
...
/**
 * 构建单表的更新语句
 * @param string $setpre Set之前的语句,包含set
 * @param array $setable 字段表,如果为字符串,chr(1)开头的原样写入,不box
 * @param type $wheresuf where之后的语句 ,包含where
 * @param array $ExcludeKeys $setable中排除的字段
 * @return string
 */
function BuildSingleTableUpdateSqlExp($setpre,array &$setable,$wheresuf,array $ExcludeKeys=NULL){
    if(empty($setable)) return NULL;
    $cmd = '';
    foreach ($setable as $k => $v)
    {
        ...
        switch (gettype($v))
        {
            case 'string':
                if($v{0}!==chr(1))
                  $cmd.= '\'' . mysql_real_escape_string($v) . '\'';
                else
                  $cmd.= substr($v,1) ;
                break;
            case 'boolean':
            ...
            case 'NULL':
                ...
                break;            
            case 'array':
                $cmd.='\'' . mysql_real_escape_string(implode(',', $v)) . '\'';
                break;
            default:
                $cmd.=strval($v);
                break;
    ...
    if(empty($cmd)) return NULL;
    $cmd = $setpre.' '.$cmd;
    $cmd = substr($cmd,0,-1).' ';//删除最后一个,
    $cmd.= $wheresuf;    
    return $cmd;
}
/**
 * 构建单表的插入语句
 * @param type $tableallname 表全名
 * @param array $values 字段与值列表,如果为字符串,chr(1)开头的原样写入,不box
 * @param type $fromsuf 如果为有条件语句的插入,则必须如右格式: 'FROM hgh_enewsdo WHERE *** limit 1'
 * @param array $ExcludeKeys  $values中排除的字段
 * @param string $insertexp 插入动作语句  
 * @return string|null
 */
function BuildSingleTableInsertSqlExp($tableallname,array &$values,$fromsuf=NULL,array $ExcludeKeys=NULL,$insertexp = 'INSERT INTO'){
   if(empty($values)) return NULL; 
   $cmd = $insertexp.' `'.$tableallname.'` ';
   $fields = '';
   $vlist ='';
   foreach ($values as $k => $v)
    {
        ...
        switch (gettype($v))
        {
            ...
            case 'NULL':
                ...
                break;            
            case 'array':
                $vlist.='\'' . mysql_real_escape_string(implode(',', $v)) . '\'';
                break;
            default:
                $vlist.=strval($v);
                break;
    ...
    $vlist = substr($vlist,0,-1);//删除最后一个,
    if(empty($fields)) return NULL;
    if(empty($fromsuf))
    {
      $cmd.=' ('.  $fields .') VALUES ('.$vlist.')';
    }
    else
    {
        $cmd.=' ('.  $fields .') SELECT '.$vlist.' '.$fromsuf;
    }
    return $cmd;
}
 ...
 * @return boolean
 */
function ExistsPubVar($var)
{
    global $dbtbpre, $empire;
    if (empty($var))
        return FALSE;
    $r = $empire->fetch1("select count(1) from {$dbtbpre}enewspubvar where myvar='{$var}'", MYSQL_NUM);
    if (empty($r) || intval($r[0]) < 1)
        return FALSE;
    return TRUE;
}

/**
 * 增加扩展变量(LGM 增加[2014年3月12日10:54])
 * 如果已经有,则更新
 * @param array $add : array('myvar','varvalue','varname','classid','myorder','varsay','tocache')
 * @param bool $reloadcfg 是否重新加载配置
 * @return boolean
 */
function AddToPubVar($add,$reloadcfg=TRUE) 
 {
    global $dbtbpre, $public_r, $empire;
    $add['myvar'] = mysql_real_escape_string($add['myvar']);
    if (!$add['myvar'] || !$add['varvalue'])
        return FALSE;
    if (empty($add['varname']))
        $add['varname'] = $add['myvar'];
    $classid = 0;
    if (!empty($add['classid']))
        $classid = (int) $add['classid'];
    $myorder = 0;
    if (!empty($add['myorder']))
        $myorder = (int) $add['myorder'];
    $tocache = 1;
     if (isset($add['tocache']))
        $tocache = (int) $add['tocache'];   
    $add['varvalue'] = mysql_real_escape_string($add['varvalue']);
    if (empty($add['varsay']))
        $add['varsay'] = '';
    $r = $empire->fetch1("select varid from {$dbtbpre}enewspubvar where myvar='{$add['myvar']}'",MYSQL_ASSOC);
    $gsql = '';
    if (empty($r))
        $gsql = "insert into {$dbtbpre}enewspubvar(myvar,varname,varvalue,varsay,myorder,classid,tocache) values('{$add['myvar']}','{$add['varname']}','{$add['varvalue']}','{$add['varsay']}',$myorder,$classid,$tocache);";
    else
        $gsql = "update {$dbtbpre}enewspubvar set varname='{$add['varname']}',varvalue='{$add['varvalue']}',varsay='{$add['varsay']}',myorder=" . $myorder . ",classid=" . $classid . ",tocache=" . $tocache . " where varid=" . $r['varid'];
    $sql = $empire->query($gsql);
    if ($sql) {
        if (!!$reloadcfg && function_exists('GetConfig'))
            GetConfig();
        return TRUE;
    }
    return FALSE;
...
/**
 * 取/e/的真实路径
 * @author LGM [2014年3月12日21:38]
 * @param bool $isFilePath 是否作为文件使用
 * @return string 以DIRECTORY_SEPARATOR(或者/)结尾
 */
function GetEPath($isFilePath=FALSE)
{
    $fpa = __FILE__;
    if(((bool)$isFilePath) == FALSE)
       $fpa =substr(__FILE__,strlen(ECMS_PATH)-1);//ECMS_PATH 最后一个字符是DIRECTORY_SEPARATOR
    $xp= strripos($fpa,DIRECTORY_SEPARATOR.'class'.DIRECTORY_SEPARATOR);
    $fpa = substr($fpa,0,$xp+1);//以DIRECTORY_SEPARATOR结尾
    if((bool)$isFilePath) return $fpa;
    else  $fpa = str_replace ('\\', '/',$fpa);
    return $fpa;    
}
/**
 * 计算中文字符串长度(LGM添加[2014年3月31日22:14])
 * @param string $string
 * @return int
 */
