...
//返回标题是否重复
function ReturnCheckRetitle(&$add){//LGM 修改，参数为引用传递[2014年2月25日9:03]
	global $empire,$dbtbpre,$class_r;
...
/**
 * 增加/修改信息处理变量(LGM 修改,在修改时如果字段不存在,则不处理[2014年3月4日8:07])
 * @global array $class_r
 * @param array $add
 * @param int $mode 0为新增模式,1为修改模式
 * @return array
 */
function &DoPostInfoVar(array &$add,$mode=0){
	global $class_r;
	//组合标题属性
	if(isset($add['titlecolor']) || $mode==0) $add['titlecolor']=RepPhpAspJspcodeText($add['titlecolor']);
	if(isset($add['titlefont']) || isset($add['titlecolor']) || $mode==0) $add['my_titlefont']=TitleFont($add['titlefont'],$add['titlecolor']);
	//专题
	if(isset($add['ztids']) || $mode==0) $add['ztids']=RepPostVar($add['ztids']);
	if(isset($add['zcids']) || $mode==0) $add['zcids']=RepPostVar($add['zcids']);
	if(isset($add['oldztids']) || $mode==0) $add['oldztids']=RepPostVar($add['oldztids']);
	if(isset($add['oldzcids']) || $mode==0) $add['oldzcids']=RepPostVar($add['oldzcids']);
	//其它变量
	if(isset($add['keyboard']) || $mode==0) $add['keyboard']=RepPhpAspJspcodeText(DoReplaceQjDh($add['keyboard']));
	if(isset($add['titleurl']) || $mode==0) $add['titleurl']=RepPhpAspJspcodeText($add['titleurl']);
	if(isset($add['checked']) || $mode==0) $add['checked']=(int)$add['checked'];
	if(isset($add['istop']) || $mode==0) $add['istop']=(int)$add['istop'];
	if(isset($add['dokey']) || $mode==0) $add['dokey']=(int)$add['dokey'];//?
	if(isset($add['isgood']) || $mode==0) $add['isgood']=(int)$add['isgood'];
	if(isset($add['groupid']) || $mode==0) $add['groupid']=(int)$add['groupid'];
	if(isset($add['newstempid']) || $mode==0) $add['newstempid']=(int)$add['newstempid'];
	if(isset($add['firsttitle']) || $mode==0) $add['firsttitle']=(int)$add['firsttitle'];
	if(isset($add['userfen']) || $mode==0) $add['userfen']=(int)$add['userfen'];
	if(isset($add['closepl']) || $mode==0) $add['closepl']=(int)$add['closepl'];//?
	if(isset($add['ttid']) || $mode==0) $add['ttid']=(int)$add['ttid'];
	if(isset($add['oldttid']) || $mode==0) $add['oldttid']=(int)$add['oldttid'];
	if(isset($add['onclick']) || $mode==0) $add['onclick']=(int)$add['onclick'];
	if(isset($add['totaldown']) || $mode==0) $add['totaldown']=(int)$add['totaldown'];
	if(isset($add['infotags']) || $mode==0) $add['infotags']=RepPhpAspJspcodeText(DoReplaceQjDh($add['infotags']));
	if(isset($add['titlepic']) || $mode==0) $add['ispic']=$add['titlepic']?1:0;
	if(isset($add['filename']) || $mode==0) $add['filename']=RepFilenameQz($add['filename'],1);
	if(isset($add['newspath']) || $mode==0) $add['newspath']=RepFilenameQz($add['newspath'],1);
	if(isset($add['titleurl']) || $mode==0) $add['isurl']=$add['titleurl']?1:0;
	return $add;
}
...
function AddNews($add,$userid,$username){
	...
	//副表
	$diy_cmd="insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r['datafields'].") values('$id','$add[classid]','$keyid','$add[dokey]','$add[newstempid]','$add[closepl]',0,'".addslashes($add[infotags])."'".$ret_r['datavalues'].");";
	$finfosql=$empire->query($diy_cmd);
	//更新栏目信息数
...
function EditNews($add,$userid,$username){
	...
	if($add['classid']==$add['oldclassid']||empty($add['oldclassid'])){
                $ccr=$empire->fetch1("select classid,modid,listdt,haddlist,sametitle,addreinfo,wburl,repreinfo from {$dbtbpre}enewsclass where classid={$add['classid']} and islast=1 limit 1",MYSQL_ASSOC);
	}else{
                $ccr=$empire->fetch1("select classid,modid,listdt,haddlist,sametitle,addreinfo,wburl,repreinfo from {$dbtbpre}enewsclass where classid={$add['oldclassid']} and islast=1 limit 1",MYSQL_ASSOC);
	}
	if(!$ccr['classid']||$ccr['wburl'])
	{
		printerror("ErrorUrl","history.go(-1)");
	...
	//索引表
        $index_checkr=$empire->fetch1("select id,classid,checked from {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']."_index where id={$add['id']} limit 1",MYSQL_ASSOC);
            if(!$index_checkr['id'])
            {
                    printerror("ErrorUrl","history.go(-1)");
            }            
	...
	$infotb=ReturnInfoMainTbname($class_r[$add['classid']]['tbname'],$index_checkr['checked']);
        $checkr=$empire->fetch1("select id,classid,userid,username,ismember,stb,newspath,filename,isqf,fstb,isgood,firsttitle,istop,groupid,isurl,titleurl from ".$infotb." where id={$add['id']} limit 1",MYSQL_ASSOC);
	...
	$pubcheckr=$empire->fetch1("select copyids from {$dbtbpre}enewsinfovote where pubid='$pubid' limit 1",MYSQL_ASSOC);
	$mid=$class_r[$add['classid']]['modid'];
	$pf=$emod_r[$mid]['pagef'];
	$add=DoPostInfoVar($add,1);//返回变量
	$add['fstb']=$checkr['fstb'];
	$ret_r=ReturnAddF($add,$class_r[$add['classid']]['modid'],$userid,$username,1,0,1);//返回自定义字段
	$deloldfile=0;
	if($add['groupid']<>$add['oldgroupid']||($index_checkr['checked'] && isset($add['checked']) && !$add['checked'] ))//改变文件权限
	{
	...
	{
                $qfr=$empire->fetch1("select checktno from {$dbtbpre}enewswfinfo where id={$add['id']} and classid={$add['classid']} limit 1",MYSQL_ASSOC);
		if($qfr['checktno']=='100' && isset($add['checked']))//已通过
		{
	...
	if(isset($add['newspath']) && $add['newspath']!=$checkr['newspath'])//如果没有设置$add['newspath'],则不需要处理
	{
	...
	if($add['info_diyotherlink'])
	{
		if(isset($add['info_keyid'])) $keyid=DoPostDiyOtherlinkID($add['info_keyid']);
	}
	else
	{
		if(isset($add['keyboard'])) $keyid=GetKeyid($add['keyboard'],$add['classid'],$add['id'],$class_r[$add['classid']]['link_num']);
	}
	//附加链接参数
	$addecmscheck=empty($newchecked)?'&ecmscheck=1':'';
	//信息地址
	$infourl=GotoGetTitleUrl($add['classid'],$add['id'],$urlnewspath,$urlfilename,(isset($add['groupid'])?$add['groupid']:$checkr['groupid']),(isset($add['isurl'])?$add['isurl']:$checkr['isurl']),(isset($add['titleurl'])?$add['titleurl']:$checkr['titleurl']));
	if(($add['classid']<>$add['oldclassid'] || $add['oldchecked']<>$add['checked']) && empty($add['titleurl']))    $add['titleurl'] = $infourl;
        //返回表信息
	$infotbr=ReturnInfoTbname($class_r[$add['classid']]['tbname'],$index_checkr['checked'],$checkr['stb']);
	//索引表
        $updparams = array();
        BuildUpdatePA($updparams,'newstime',$newstime);
        BuildUpdatePA($updparams,'classid',$add['classid']);
        BuildUpdatePA($updparams,'lastdotime',$lastdotime);     
        $vcmd = SimpleJoinUpdateSQL("update {$dbtbpre}ecms_".$class_r[$add['classid']]['tbname']."_index set", $updparams, "where id={$add['id']} limit 1",$aqf);
        $indexsql=$empire->query($vcmd);
	//主表
        $updparams = array();
        BuildUpdatePA($updparams,'classid',$add['classid'],0);
        BuildUpdatePA($updparams,'ttid',$add['ttid'],0);
        BuildUpdatePA($updparams,'onclick',$add['onclick'],0);
        BuildUpdatePA($updparams,'totaldown',$add['totaldown'],0);
        BuildUpdatePA($updparams,'firsttitle',$add['firsttitle'],0);//是否头条
        BuildUpdatePA($updparams,'isgood',$add['isgood'],0);
        BuildUpdatePA($updparams,'ispic',$add['ispic'],0);
        BuildUpdatePA($updparams,'istop',$add['istop'],0);
        BuildUpdatePA($updparams,'isurl',$add['isurl']);
        BuildUpdatePA($updparams,'lastdotime',$lastdotime);
        BuildUpdatePA($updparams,'groupid',$add['groupid'],0);
        BuildUpdatePA($updparams,'userfen',$add['userfen'],0);
        if(isset($add['my_titlefont']))BuildUpdatePA($updparams,'titlefont',addslashes($add['my_titlefont']));
        if(!empty($add['titleurl']))BuildUpdatePA($updparams,'titleurl',addslashes($add['titleurl']));
        if(isset($add['keyboard']))BuildUpdatePA($updparams,'keyboard',addslashes($add['keyboard']));
        $vcmd = SimpleJoinUpdateSQL("update ".$infotbr['tbname']." set", $updparams, "where id={$add['id']} limit 1",$updatefile.$ret_r['values']);
	$sql=$empire->query($vcmd);
	//副表
	$stb=$checkr['stb'];
        $updparams = array();
        BuildUpdatePA($updparams,'classid',$add['classid'],0);
        BuildUpdatePA($updparams,'keyid',$keyid);
        BuildUpdatePA($updparams,'dokey',$add['dokey']);
        BuildUpdatePA($updparams,'newstempid',$add['newstempid'],0);
        BuildUpdatePA($updparams,'closepl',$add['closepl'],0);
        if(isset($add['infotags']))BuildUpdatePA($updparams,'infotags',addslashes($add['infotags']));
        $vcmd = SimpleJoinUpdateSQL("update ".$infotbr['datatbname']." set", $updparams, "where id={$add['id']} limit 1",$ret_r['datavalues']);
	$fsql=$empire->query($vcmd);
	//取第一张图作为标题图片
	if($add['getfirsttitlepic']&&empty($add['titlepic']))
	{
	...
	//投票
        if(isset($add['vote_name']))AddInfoVote($add['classid'],$add['id'],$add); //2014年3月3日11:20 LGM 修改,如果没有请求更改,则不需要执行
	//写入专题
	if(isset($add['ztids'])) InsertZtInfo($add['ztids'],$add['zcids'],$add['oldztids'],$add['oldzcids'],$add['classid'],$add['id'],$newstime);//2014年3月3日11:20 LGM 修改,如果没有请求更改,则不需要执行
	//TAGS
	if($add['infotags'] && $add['infotags']<>$add['oldinfotags'])
	{
	...
	if($sql)
	{
		...
		else
		{
                ...
                $appdata = array('id'=>$add['id']);
		printerror("EditNewsSuccess",$ecmsfrom.$addecmscheck,$appdata);
	}
	else
	{
...
function EditInfoSimple($add,$userid,$username){
	...
	if($sql)
	{
		...
		insert_dolog("classid=$add[classid]<br>id=".$add[id]."<br>title=".$add[title],$pubid);//操作日志
                $appdata = array('id'=>$add['id']);
		printerror("EditNewsSuccess",$closeurl,8,$appdata);//LGM 修改,增加附加数据[2014年3月4日11:35]
	}
	else
	{
