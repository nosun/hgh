...
function AddPlDataTable($add,$userid,$username){
	...
	$tr=$empire->fetch1("select pldatatbs from {$dbtbpre}enewspl_set limit 1");
	...
	//建表
	$odtb=$dbtbpre."enewspl_1";
	$dtb=$dbtbpre."enewspl_".$datatb;
	CopyEcmsTb($odtb,$dtb);
	$sql=$empire->query("update {$dbtbpre}enewspl_set set pldatatbs='$newdatatbs' limit 1");
	GetConfig();//更新缓存
	if($sql)
	{
		//操作日志
...
function DefPlDataTable($add,$userid,$username){
	...
	$sql=$empire->query("update {$dbtbpre}enewspl_set set pldeftb='$datatb' limit 1");
	GetConfig();//更新缓存
	if($sql)
	{
		//操作日志
...
function DelPlDataTable($add,$userid,$username){
	...
	if($public_r['pldeftb']==$datatb||$datatb==1)
	{
	...
	$sql=$empire->query("update {$dbtbpre}enewspl_set set pldatatbs='$newdatatbs' limit 1");
	//删除评论
	$plsql=$empire->query("select plid,classid,id,pubid from {$dbtbpre}enewspl_{$datatb}");
	while($plr=$empire->fetch($plsql))
	{
		$tbname=$class_r[$plr[classid]][tbname];
		if($tbname&&$plr['pubid']>0)
		{
			$index_r=$empire->fetch1("select checked from {$dbtbpre}ecms_".$tbname."_index where id='$plr[id]' limit 1");
			//返回表
			$infotb=ReturnInfoMainTbname($tbname,$index_r['checked']);
			$empire->query("update ".$infotb." set plnum=plnum-1 where id='$plr[id]'");
		}
	}
	//删除表
	$deltb=$empire->query("DROP TABLE IF EXISTS {$dbtbpre}enewspl_".$datatb.";");
	GetConfig();//更新缓存
	if($sql)
	{
		//操作日志
