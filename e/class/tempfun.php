...
function EditLoginIframe($temptext,$userid,$username){
	...
	{
		$r=ReLoginIframe();
                if($r!==TRUE && isset($r['error']))//LGM修改,如果生成过程出错,则不保存入数据库,返回报错[2014年3月16日21:17]
                {
                    $p = array('msg'=>strval(count($r['error'])).'个错误');
                    $description = '';
                    ...
                    $p['waittime'] = 12;
                    printerror("ExceptionSuspend","template/EditPublicTemp.php?tname=loginiframe&gid=$gid#loginiframe",$p);
                }
	}        
	...
	if($gid==$public_r['deftempid']||(!$public_r['deftempid']&&($gid==1||$gid==0)))
	{
		$r = ReLoginIframe();
                if($r!==TRUE && isset($r['error']))//LGM修改,如果生成过程出错,则不保存入数据库,返回报错[2014年3月16日21:17]
                {
                    ...
                    $p['description'] = $description;
                    $p['waittime'] = 12;
                    printerror("ExceptionSuspend","template/EditPublicTemp.php?tname=loginjstemp&gid=$gid#loginjstemp",$p);
                }
	}        
	...
	if($gid==$public_r['deftempid']||(!$public_r['deftempid']&&($gid==1||$gid==0)))
	{
		...
		elseif($tempname=="pljstemp")
		{
			$r = GetPlJsPage();
                ...
                if($r!==TRUE && isset($r['error']))//LGM 修改,如果发生错误,则不执行数据库的更新,返回报错.
                {
                    $p = array('msg'=>strval(count($r['error'])).'个错误');
                    $description = '';
                    foreach($r['error'] as $e)
                    ...
                    $p['description'] = $description;
                    $p['waittime'] = 12;
                    printerror("ExceptionSuspend","template/EditPublicTemp.php?tname=$tempname&gid=$gid#$tempname",$p);
                }      
	}
...
function EditIndextemp($temptext,$userid,$username){
	...
	if($gid==$public_r['deftempid']||(!$public_r['deftempid']&&($gid==1||$gid==0)))
	{
		$r = NewsBq($classid,eaddslashes($temptext),1,0);                
                if(is_array($r) && isset($r['error']))//LGM修改,如果有错误,则不更新数据库,并且返回报错
                {
                    $p = array('msg'=>strval(count($r['error'])).'个错误');
                    $description = '';
                    foreach($r['error'] as $e)
                    ...
                    $p['description'] = $description;
                    $p['waittime'] = 12;
                    printerror("ExceptionSuspend","template/EditPublicTemp.php?tname=indextemp&gid=$gid",$p);
                }
		//删除动态模板缓存文件
