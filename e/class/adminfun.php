...
function login($username,$password,$key,$post){
	...
	$user_r=$empire->fetch1("select userid,password,salt,lasttime,lastip,addtime,addip,userprikey from {$dbtbpre}enewsuser where username='".$username."' and checked=0 limit 1",MYSQL_ASSOC);
	if(!$user_r['userid'])
	{
	...
	//安全问答
	$user_addr=$empire->fetch1("select userid,equestion,eanswer,openip,certkey from {$dbtbpre}enewsuseradd where userid='$user_r[userid]'",MYSQL_ASSOC);
	if(!$user_addr['userid'])
	{
		InsertErrorLoginNum($username,$password,0,$loginip,$logintime);
		printerror("LoginFail","index.php");
	...
	$r=$empire->fetch1("select groupid,userid,styleid,userprikey from {$dbtbpre}enewsuser where username='$username' limit 1",MYSQL_ASSOC);
	//样式
	if(empty($r[styleid]))
	{
	...
	{
		$set5=delcookie('ecmsdodbdata',1);//LGM修改,统一删除方式
	}
	
	ecmsEmptyShowKey($keyvname,0);//清空验证码
	$set4=esetcookie("loginuserid",$r['userid'],0,1,TRUE);//LGM修改,加密方式[2014年3月17日14:39]
...
function loginout($userid,$username,$rnd){
	...
	$set1=delcookie("loginuserid",1);//LGM修改,统一删除方式
	$set2=delcookie("loginusername",1);
	$set3=delcookie("loginrnd",1);
	$set4=delcookie("loginlevel",1);
	//COOKIERND
	DelECookieRnd();
	DelESessionRnd();
	//FireWall
