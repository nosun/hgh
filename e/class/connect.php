...
function printerror($error = "", $gotourl = "", $ecms = 0, $noautourl = 0, $novar = 0, array $metadata = array())
{
    ...
    if ($argc > 0) {
        ...
        if (is_array($lastarg)) {
            ...
            if (isset($lastarg['error'])) {
                $error = $lastarg['error'];
                unset($lastarg['error']);
            }
            if (isset($lastarg['gotourl'])) {
                $gotourl = $lastarg['gotourl'];
                unset($lastarg['gotourl']);
            }
            if (isset($lastarg['ecms'])) {
                $ecms = $lastarg['ecms'];
                unset($lastarg['ecms']);
            }
            if (isset($lastarg['noautourl'])) {
                $noautourl = $lastarg['noautourl'];
                unset($lastarg['noautourl']);
            }
            if (isset($lastarg['novar'])) {
                $novar = $lastarg['novar'];
                unset($lastarg['novar']);
            }
...
function ajax_printerror($result = '', $ajaxarea = 'ajaxarea', $error = '', $ecms = 0, $novar = 0, array $metadata = array())
{
    ...
    if ($argc > 0) {
        ...
        if (is_array($lastarg)) {
            ...
            if (isset($lastarg['result'])) {
                $result = $lastarg['result'];
                unset($lastarg['result']);
            }
            if (isset($lastarg['ajaxarea'])) {
                $ajaxarea = $lastarg['ajaxarea'];
                unset($lastarg['ajaxarea']);
            }
            if (isset($lastarg['ecms'])) {
                $ecms = $lastarg['ecms'];
                unset($lastarg['ecms']);
...
function DoTranFile($file, $file_name, $file_type, $file_size, $classid, $ecms = 0)
{
    ...
    //  $r[savepath] = 'http://115.28.44.98/p/'. $filepath;
    //附件地址
...
function GetEPath($isFilePath = FALSE)
{
    ...
    if (((bool)$isFilePath) == FALSE)
        $fpa = substr(__FILE__, strlen(ECMS_PATH) - 1);
    //ECMS_PATH 最后一个字符是DIRECTORY_SEPARATOR
...
/**
 * 自定义字符截取(红星 2014-12-26 14:30:20)
 * @param string $string  //要处理的字符串
 * @param int $length  //截取中文字数
 * @return string 
 */

function usr_esub($string, $length){
    $string = strip_tags($string);
    $string = esub($string,$length);
