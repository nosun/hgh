...
require_once(QQXCLASS_PATH.'Oauth.class.php');
...
//</editor-fold> 
 ...
 /**
   ...
   public static function GetCommInvokeParamFromSession($access_token, $app_id=NULL) {
        ...
        {
             $myps= $this->ConvertParam($params,__FUNCTION__);
       ...
       /**
	 * 重载的构造函数
	 * @param array $params : array("应用号","应用密钥","访问令牌")|array(chr(1)."client_id",chr(1)."client_secret",chr(1)."access_token")|array('id')
	 */
        ...
        if (!empty($myps)) {
            if (isset($myps ['client_id']) && isset($myps ['client_secret'])) {
                return $this->__construct(
                        ...
                        (isset($myps ['oauth']) ? $myps['oauth'] : NULL)
                );
            }
 ...
 * @param EOAuth $oauth 注意:请传入引用
 * @return QQAction
 */
     public function __construct($client_id_obj, $client_secret = NULL, $access_token = NULL, $redirect_uri = NULL, EOAuth $oauth = NULL){
        ...
        if (empty($oauth)) {
            if (func_num_args() == 1) {
                ...
                elseif (is_string($client_id_obj)) {
                    $appname = $client_id_obj;
                }
            }
            else
                $this->oauth = new QQOAuth($client_id_obj, $client_secret, $access_token, $redirect_uri);
        }
        else
            $this->oauth = $oauth;
        if(empty($appname) && $this->oauth)
                $appname = $this->oauth->GetAppName();
        ...
        if (is_file($pl_filename))
        {
            ...
            unset($apprw__arr);
            if (!empty($cax) && is_array($cax) && !empty($cax[$appname]))
            {
                $rreg = &$this->TranslateRegexMap();
                $rkeyw = &$this->TranslateKeyWordsMap();
                if (!empty($cax[$appname]['tranregexlist']))
                    $rreg = $cax[$appname]['tranregexlist'];
                if (!empty($cax[$appname]['trankeywordslist']))
                    $rkeyw = $cax[$appname]['trankeywordslist'];
            }
        }
        $this->keysArr = array(
    ...
    /**
     * 调用相应api
     * @param array $arr $arr['_发送方式_'] == -1:用EIService::oAuthRequest发送
     * @param type $argsList
     * @param type $baseUrl
     * @param type $method
     * @return type
    ...
    private function _applyAPI($arr, $argsList, $baseUrl, $method){
        ...
        elseif($send_type==-1){
              $response =  $this->oauth->GetEiService($this->oauth)->CurlRequest($baseUrl, $method, $keysArr,$haveFile);
        }
        return $response;
     ...
     * @param array $arg      参数列表数组,$arg['_发送方式_'] == -1:用EIService::oAuthRequest发送
     * @since 5.0
 ...
 */
   ...
   public function GetSingleInfo(array &$params)
   {
       ...
       if (isset ( $myps ) && ! empty ( $myps )) {
            ...
            $r = $result['tweetinfo'];
            return  $this->ConvertParam($r,__FUNCTION__,($params['ForbidConvertOut']?-1:1));
           }
           else
           {
    ...
    public function CreateInfo(array &$params) {
        ...
        if (isset($myps) && !empty($myps)) {
            ...
            if (!empty($myps['playurl']) && strlen($myps['playurl']) > 255) {
                ...
                $sr = $this->ShortUrl($mmp);
                if ($sr) {
                    $vlurl = $myps['playurl'] = $sr['s'];
                }
            }
            if (!empty($myps['playurl']) && empty($myps['type'])) {
                $myps['type'] = 5;
            }
            if (empty($myps['site'])) {
                $myps['site'] = $public_r['sitename'];
            }
            if (empty($myps['fromurl'])) {
                $myps['fromurl'] = $public_r['newsurl'];
            }
            $r = $this->__call('add_share', $myps);
            if (!empty($r) && in_array($r['ret'], array(100031, 3046, 3034, 3028))) {
                //组织微博的内容:参数content
                $myps = $this->ConvertParam($params, __FUNCTION__); //重新赋值
                if(empty($myps['images']) || !$allowUpPic) {
                    ...
                    return $this->CreateWeibo($myps);
                }
                else
                {
                    //发图片微博,先转换参数:
                    $txt = $myps['comment'];
                    $txt .= '[评论:' . $myps['title'] . ' ' . $myps['url'] . ']';
                    ...
                    if ($rpos !== FALSE) { //是当前域名下的文件:
                        ...
                        $picfpath = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'e'. DIRECTORY_SEPARATOR . 'data'. DIRECTORY_SEPARATOR . 'tmp'. DIRECTORY_SEPARATOR .uniqid().'.'.pathinfo(parse_url($mypic, PHP_URL_PATH), PATHINFO_EXTENSION);
                        $fp = fopen($picfpath, "w");
                        if (fwrite($fp, $picc)) {
                            fclose($fp);
                            sleep(1);
...
public function DestroyFriendships(array &$params)
{
	 ...
	 * @return string
	 */
	public function TruncateContent($str,$submode) {
		if (empty ( $str ))
			return '';
		if (strlen ( $str ) < self::MaxInfoLen($submode))
			return $str;
		$pn = '/\b(http(s)?:\/\/\S+)/i';
		$pn2 = '/\b(http(s)?:\/\/\S+)$/i';
		$pn3 = '|\bhttp://url\.cn/XXXXXXXX|i';//http://url.cn/DO7mxs
		...
		$pstr = EIService::cn_substr ( $rstr, self::MaxInfoLen($submode), 0,  $submode); // 截取前N个字符
