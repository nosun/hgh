...
function AdminReturnClassLink($classid){
	...
	if(empty($class_r[$classid][featherclass]))
	{
        ...
        $adminpath = '/e/'.  GetValueFromCookie('pathn',1,FALSE,'admin').'/';//LGM增加,确定后台路径[2014年3月12日20:25]
...
function GetHtml($classid,$id,$add,$ecms=0,$doall=0){
	...
	//副表
	$addr=$empire->fetch1("select ".ReturnSqlFtextF($mid)." from {$dbtbpre}ecms_".$tbname."_data_".$add[stb]." where id='$add[id]' limit 1",MYSQL_ASSOC);
...
function DoECreatFileRnd($userid,$username,$rnd,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	...
	$file=ECMS_PATH.'e/data/adminlogin/user'.$userid.'_'.md5(md5($username.'-empirecms!check.file'.$truelogintime.'-'.$rnd.$ecms_config['esafe']['ecookiernd']).'-'.$ip.'-'.$userid.'-'.$rnd.$adminstyle.'-'.$groupid.'-'.$dbdata.$sessval).'.log';
        return (bool)WriteFiletext_n($file,'EmpireCMS');//LGM修改,返回是否已经写入
}
...
function ChMustAddF($mid,$f,$value){
	...
	if(strstr($emod_r[$mid]['mustqenterf'],','.$f.','))
	{
		if(!trim($value))
		{
			$metadata = array('FieldName'=>ChGetFname($mid,$f),'Field'=>$f);//LGM Edit [2014年3月9日9:12]
...
function ReturnAddF($add,$modid,$userid,$username,$do=0,$rdata=0,$ch=0){
	...
	elseif($do==1)//修改
	{
		...
		{
			$f=$r[$i];
			if($f=="special.field"||!strstr($emod_r[$modid]['caneditf'],','.$f.',') || (($f=="newstext" || $f=="smalltext")  && !isset($add[$f])))//LGM 修改,如果概述或者内容没有设置,则不记入语句[2014年3月3日17:24]
			{
				continue;
			}
			$add[$f]=ReturnCheckboxAddF($add[$f],$modid,$f);//复选框
