...
if($action == 'test') {
	...
	if($ur['userid'])
	{
		...
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
                esetcookie('lastlogin',time().'@'.$ur['userid'],time()+7776000,0,TRUE);//网站登录的最后时间,以标识已经注册了账户.保存三个月(LGM添加[2014年3月18日9:31])
		$set1=esetcookie("mlusername",$ur['username'],$logincookie);
		$set2=esetcookie('mluserid',$ur['userid'],$logincookie,0,TRUE);//LGM修改,把用户ID加密[2014年3月16日21:43]
		$set3=esetcookie("mlgroupid",$ur['groupid'],$logincookie);
		$set4=esetcookie("mlrnd",$rnd,$logincookie);
		delcookie('mldoactive',0);//LGM修改,统一删除方式
	...
	//同步登出 API 接口
	header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
	$set1=delcookie("mlusername");//LGM修改[2014年3月16日21:44]
	$set2=delcookie("mluserid");
	$set3=delcookie("mlgroupid");
	$set4=delcookie("mlrnd");
	$set5=delcookie("mlauth");
