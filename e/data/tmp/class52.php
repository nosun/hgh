...
<html xmlns:wb="http://open.weibo.com/wb" xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>红色中国 - <?=$public_r['sitename']?></title>
    ...
    <div class="redlogo">
        <div class="logo"><a href="http://www.szhgh.com/" title="红歌会网首页"><img src="http://www.szhgh.com/skin/default/images/logo.png" /></a></div>
            ...
            <!-- class navigation -->
            <div class="navwrap">
                <div id='js_controlclassid' classid='52' class="navigation">
...
<script type="text/javascript" src="http://cpro.baidustatic.com/cpro/ui/c.js"></script>
                ...
                <div class="main">
                    <div class="articleA">
                        <div class="section pictitle left">
                            <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('selfinfo',1,0,1,'','newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
                            <a href="<?=$bqsr['titleurl']?>" title="<?=$bqr['title']?>" target="_blank"><img src="<?=sys_ResizeImg($bqr['titlepic'],310,232,1,'')?>" /></a>
                            <h1><a href="<?=$bqsr['titleurl']?>" title="<?=$bqr['title']?>" target="_blank"><?=$bqr['title']?></a></h1>
                            <?php
}
}
?>
...
?>
                                <li class="<?if($bqno==1){echo 'first';}elseif($bqno==3){echo 'last';}?>">
                                    ...
                                    <div class='smalltext'>
                                        <p><?=htmlspecialchars_decode(esub(strip_tags($bqr['smalltext']),120))?> <span class="readmore"><a href="<?=$first_r['titleurl']?>" title="点此阅读全文" target="_blank">[全文]</a></span></p>
                                    </div>
                                </li>
                                <?php
}
...
?>
                    ...
                    <div class="articleB">
                        ...
                        $this_query=$empire->query("select classid, sonclass, classname, classpath from `hgh_enewsclass` where bclassid='$bclassid' order by `classid` ASC");
                        while($bqr=$empire->fetch($this_query)) {
                            if($bqr[sonclass]){
                                $son_r=  explode("|", $bqr[sonclass]);
                                $count=  count($son_r);
                                for($a=1;$a<$count-1;$a++){
                                    if($a==$count-2){
                                        $in_classid.=$son_r[$a];
                        ...
                        <div class="section">
                            ...
                            <div class="section_content">
                                <div class="left firsttitle">
                                    ...
                                    <?php
                                    //取得该分类下的有标题图片的数据
                                    $first_query=$empire->query("select title,titlepic,smalltext,titleurl from `hgh_ecms_article` where classid in( ".$in_classid.") and ispic=1 order by newstime desc limit 2");
                                    $i=0;
                                ...
                                <ul class="right list">
                                    <?php
                                    //取得该分类下的最新数据
                                    $newbook=$empire->query("select title,titlepic,smalltext,titleurl,keyboard,newstime,groupid,classid,newspath,filename,id from hgh_ecms_article  where classid in(".$in_classid.") order by newstime desc limit 11");
...
<script src=http://www.szhgh.com/e/public/onclick/?enews=doclass&classid=52></script>
    </body>
</html>