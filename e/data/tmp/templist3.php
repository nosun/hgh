...
<head>
	...
	<link href="[!--news.url--]skin/default/css/mF_pithy_tb.css" rel="stylesheet" type="text/css" />
	<script type="text/javascript" src="[!--news.url--]skin/default/js/jquery-1.8.2.min.js"></script>
	<script type="text/javascript" src="[!--news.url--]skin/default/js/myfocus-2.0.4.min.js"></script>		
	<script type="text/javascript" src="[!--news.url--]skin/default/js/mF_pithy_tb.js"></script>
    ...
    <div class="redlogo">
        <div class="logo"><a href="[!--news.url--]" title="红歌会网首页"><img src="[!--news.url--]skin/default/images/logo.png" /></a></div>
            ...
            <div class="container">
				...
				<div id="picfocus" class="clearfix">
					<div id="myFocus">
					  <div class="pic">
						<ul>
							<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq(101,4,2,1,'isgood>0','newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
								<li><a href="<?=$bqsr['titleurl']?>" target="_blank"><img src="<?=$bqr['picurl']?>" thumb="<?=$bqr['titlepic']?>" alt="<?=$bqr['title']?>" /></a></li>
							<?php
}
}
?>
					...
					<div id="newpic">
						...
						<ul>
							<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq(101,3,0,1,'','newstime DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
...
?>
				...
				<div id="hotpic">
					...
					<ul class="clearfix">
						<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq(101,4,0,1,'','onclick  DESC');
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
?>
							<li>
								<a href="<?=$bqsr['titleurl']?>" target="_blank"><img src="<?=$bqr['titlepic']?>" alt="<?=$bqr['title']?>" /></a>									
								<p><a href="<?=$bqsr['titleurl']?>" target="_blank"><?=$bqr['title']?></a></p>
							</li>
						<?php
}
}
