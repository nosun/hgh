...
function AddPl($username,$password,$nomember,$key,$saytext,$id,$classid,$repid,$add){
	...
	//主表
        $sql=$empire->query("insert into {$dbtbpre}enewspl_".$restb."(pubid,username,sayip,saytime,id,classid,checked,zcnum,fdnum,userid,isgood,saytext".$ret_r['fields'].") values('$pubid','".$username."','$sayip','$saytime','$id','$classid','$checked',0,0,'$muserid',0,'".mysql_real_escape_string($saytext)."'".$ret_r['values'].");");
	$plid=$empire->lastid();
	if($doaction!='dozt')
	{
		//信息表加1
...
if($sql)
 {
        //LGM 增加：第三方交互平台的信息分发 2014年2月11日23:10
        $eilist = $_POST ['selecteiv']; //格式:授权ID1|授权ID2|授权ID3...
        if (!empty($eilist)) {
            $stag = preg_split('/[\|,; ]+/', $eilist, -1, PREG_SPLIT_NO_EMPTY);
            if (!empty($stag)) {
                ...
                $cubeis = EIService::GetCUBEISForSession(); //array("id","aid","apptype","openid","ename","token",'refresh_token',"scope","appkey","appsecret","used") 当前用户绑定的第三方交互账号 
                if (!empty($cubeis)) {
                    $idx = 0;
                    foreach ($stag as $appid) {
                        $appid = intval($appid);
                        if ($appid == 0)
                            continue;
                        $apid = 0;
                        ...
                        if (empty($p))
                            continue;
                        $c = EIService::GetAction($apid, $p);
                        $params = array(); //insert_dolog($app.'||'.print_r($p,true),$muserid);
                        $params['标题'] = $_REQUEST['pagetitle'];//页面标题
                        $params['网址'] = $_REQUEST['titleurl'];//页面地址                        
                        $params['评论'] = $c->GetTranSrv()->ReplaceKeyWords($saytext);
                        $params['概述'] = $_REQUEST['csummary'];//信息摘要
                        $params['图片'] = $_REQUEST['titlepic'];//页面的主图
                        $params['媒体网址'] = $_REQUEST['playurl'];//媒体地址
                        $params['元数据'] = '[{"arcid":' . $id . ',"repid":' . $repid . '}]';
                        foreach($params as $pk => $pv){//过滤参数
                            if(empty($pv)) unset($params[$k]);
                        }                        
                        try {
                            $r = $c->CreateInfo($params);
                            $idx++;
                        }
...
function ReturnPlAddF($add,$pr,$ecms=0){
	...
	for($i=1;$i<$count;$i++)
	{
		...
		if(strstr($pr[plmustf],','.$f.','))
		{
			if(!trim($fval))
			{
				$chfr=$empire->fetch1("select fname from {$dbtbpre}enewsplf where f='$f' limit 1",MYSQL_ASSOC);
				$metadata = array('FieldName'=>$chfr['fname'],'Field'=>$f);
				printerror('EmptyPlMustF','',1,$metadata);//LGM修改[2014年2月25日10:07]
			}
		}
...
function isCanSetCookie(){
	setcookie('isCan',1);
	if(isset($_COOKIE['isCan'])){
		return true;
	}else{
		return false;
	}
