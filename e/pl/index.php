...
if($doaction=='dozt')
{
	...
	$n_r=$empire->fetch1("select ztid,ztname,intro,ztimg,ztpagekey,restb from {$dbtbpre}enewszt where ztid='$classid'",MYSQL_ASSOC);
	if(!$n_r['ztid'])
	{
		printerror("ErrorUrl","history.go(-1)",1);
	}
...
else
{
	...
	$n_r=$empire->fetch1("select * from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' limit 1",MYSQL_ASSOC);
	...
	$url=ReturnClassLink($n_r[classid])."&nbsp;>&nbsp;<a href=".$titleurl.">".$title."</a>&nbsp;>&nbsp;".$fun_r[pl];
        ...
        //生成动态页导航
        $selectFather_r=$empire->fetch1("select bclassid from {$dbtbpre}enewsclass where classid='$classid'");
...
$docomment=(int)$_GET['docomment'];
if($myorder==1)
{
	$addorder='plid';
	$search.='&myorder='.$myorder;
}
$hotquery=$query." order by zcnum+fdnum DESC limit $line";
$query.=" order by ".$addorder." limit $offset,$line";
$hotsql=$empire->query($hotquery);
