...
class usercontrol extends base {
	...
	function onedit($arr) {
        ...
        /**
         * 高级修改用户(LGM增加[2014年3月19日18:27])
         * @param array $arr
         * @return int
         */
	function onmodify($arr) {
		@extract($arr, EXTR_SKIP);//userid,username, oldpw, newpw, email, ignoreoldpw
		if(!$ignoreoldpw && $email && ($status = $this->_check_email($email)) < 0) {
			return $status;
		}
		$status = $_ENV['user']->modify_user($userid,$username, $oldpw, $newpw, $email, $ignoreoldpw);

		if($newpw && $status > 0) {
			$this->load('note');
			$_ENV['note']->add('updatepw', 'userid='.$userid.'&username='.urlencode($username).'&password='.urlencode($newpw));
		}
		return $status;
	}
