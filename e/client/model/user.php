...
class usermodel {
	...
	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0) {
        ...
        /**
         * 高级修改用户资料,包括用户名(LGM增加[2014年3月19日18:19])
         * @param int $userid
         ...
         * @return int
         */
        function modify_user($userid,$username, $oldpw, $newpw, $email, $ignoreoldpw = 0) {
                $data = $this->db->fetch_first("SELECT username, uid, password, salt FROM ".UC_DBTABLEPRE."members WHERE uid='{$userid}'");

		if($ignoreoldpw) {
                        $isprotected = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."protectedmembers WHERE uid = '{$userid}'");
			if($isprotected) {
				return -8;
			}
		}
		...
		$sqladd .= $email ? ($sqladd ? ',' : '')." email='$email'" : '';
                ...
                {
                    $muser= mysql_real_escape_string($username);
                    $sqladd .= ($sqladd ? ',' : '')." username='$muser'";
                }
		if($sqladd || $emailadd) {
                        $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET $sqladd WHERE uid='{$userid}'");
			return $this->db->affected_rows();
		} else {
			return -7;
		}
