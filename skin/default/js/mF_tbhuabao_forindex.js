...
myFocus.pattern.extend({
	'mF_tbhuabao':function(settings,$){
		...
		//CSS
		var w=settings.width,h=settings.height,dotH=212,arrTop=h/2-32,n=$txtList.length;
		$focus[0].style.cssText='width:320px;height:240px;';
		$picBox[0].style.cssText='width:'+w+'px;height:'+h+'px;';
		$picUl[0].style.width=w*2*n+'px';
		$txtList.each(function(){this.style.top=dotH+'px'});
		$picUl.find('li').each(function(){this.style.cssText='width:'+w+'px;height:'+h+'px;'});//滑动固定其大�?
