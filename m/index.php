...
if(!empty($_GET['load'])){
    ...
    $query1=$select." isgood>=1 order by newstime desc limit $offset,$line";
    ...
    while ($r = $empire->fetch($c_sql1)) {
        ...
        <section class="li<?= $vi1 ?>">
            <div class="img-text">
                ...
                <a  style="font-size: 17px" href="<?= $titleurl ?>"><b><?= $r[title] ?></b></a>
...
$query1=$select." isgood>=1 order by newstime desc limit 20";
...
?>
    <div id="slideBox" class="slideBox" style="padding:0px 5px">
...
<div class="img-text">
                        <a href="$titleurl"> <img style="float:right;" src='{$r[titlepic]}' /></a>
                    <a style="font-size: 17px" href="{$titleurl}"><b>{$r[title]}<br></b></a>
                    <p style="font-size:13px;">{$passedtimestr}  &nbsp; &nbsp;&nbsp; {$class_r[$r['classid']]['classname']}  &nbsp; &nbsp; {$r['author']}</p>
                    </div>
</section>
VX_DEND;
